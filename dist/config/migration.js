...
parentKey) {
    ...
    try {
        ...
        for (const [key, val] of Object.entries(config)) {
            if (removedOptions.includes(key)) {
            ...
            else if (key === 'extends' && is_1.default.array(val)) {
                for (let i = 0; i < val.length; i += 1) {
                    ...
                    else if (is_1.default.string(val[i]) && val[i].startsWith(':masterIssue')) {
                        isMigrated = true;
                        migratedConfig.extends[i] = val[i].replace('masterIssue', 'dependencyDashboard');
                    }
                }
            }
        ...
        logger_1.logger.debug({ config }, 'migrateConfig() error');
        throw err;
    }
}
