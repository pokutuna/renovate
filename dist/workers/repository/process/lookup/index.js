...
function getFromVersion(config, rangeStrategy, latestVersion, allVersions) {
    ...
    if (rangeStrategy === 'pin') {
        return (lockedVersion || version.maxSatisfyingVersion(useVersions, currentValue));
    }
...
async function lookupUpdates(config) {
    ...
    if (isValid) {
        ...
        const allSatisfyingVersions = allVersions.filter((v) => version.matches(v, currentValue));
        ...
        const nonDeprecatedVersions = releases
            ...
            .map((release) => release.version);
        const fromVersion = getFromVersion(config, rangeStrategy, latestVersion, nonDeprecatedVersions) || getFromVersion(config, rangeStrategy, latestVersion, allVersions);
        if (fromVersion &&
            rangeStrategy === 'pin' &&
        ...
        let filteredVersions = filter_1.filterVersions(config, filterStart, dependency.latestVersion, allVersions, releases).filter((v) => 
        ...
        for (const toVersion of filteredVersions) {
            ...
            const releaseFields = ['releaseTimestamp', 'canBeUnpublished', 'newDigest'];
